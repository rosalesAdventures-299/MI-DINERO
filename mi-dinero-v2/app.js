...
(function () {
  ...
  function openAccountEditor(acc) {
    ...
    var body = openMini(isNew ? 'Nueva cuenta' : 'Editar cuenta',
      ...
      '<div class="field-group"><span class="field-label">Saldo inicial</span><input type="text" inputmode="decimal" class="text-field" id="acc-balance" value="' + (acc ? acc.startBalance : '0') + '"/></div>' +
    ...
    $('#save-account', body).addEventListener('click', function () {
      ...
      if (isNew) {
        ...
        acc.name = name; acc.icon = chosenIcon; acc.startBalance = bal;
      }
      saveState();
      closeMiniUI();
      render();
